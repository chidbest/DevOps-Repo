...
5. Configure programmatic access from your workstation to connect to AWS using the access keys copied in No 3 above using the command `aws configure`.

![alt text](Images/5_aws_configure.png)
...
7. After configuring authentication and installing `boto3`, confirm you can programatically access your AWS account by running the below commands in `>python`. Copy them one after the order, also be aware of the indentation.
...
You shall see your previously created s3 bucket name. In my case chidi-dev-terraform-bucket

![alt text](Images/7_bucket_listed.png)
...
3. Your ability to effectively use up-to-date Terraform documentation [here](https://developer.hashicorp.com/terraform/language)
...
![alt text](<Images/7_TF_successfully init.png>)

- Notice the new `.terraform\...` directory that is created. This is where Terraform keeps plugins. Generally, it is safe to delete this folder. It just means that you must execute `terraform init` again to download them.
...
![alt text](Images/9_TF_infrastructures.png)
...
`terraform plan` - To see what terraform intends to create before we tell it to go ahead and create it by running the command

`terraform apply` - To create the public subnets.

![alt text](Images/10_vpc_created.png)

![alt text](Images/11_subnets_created.png)
...
![alt text](Images/12_terraform_destroy.png)

### Fixing the problem by Code Refactoring

**1. Fixing Hard Coded Values:** We will introduce variables, and remove hard coding. In the provider block, declare a variable named region, give it a default value, and update the provider section by referring to the declared variable.
...
**2. Fixing multiple resource blocks:** This is where things become a little tricky. We are going to introduce `Loops & Data Sources`.
...
![alt text](Images/14_TF_blockcount.png)

**3. The final problem to solve is removing the hard coded `count` value.**
...
![alt text](Images/15_TF_AZcount.png)
...
- The second part `?` and `length(data.aws_availability_zones.available.names)` means, if the first part is true, then use this. In otherwords, if preferred number of public subnets is `null` (Or not known) then set the value to the data returned by `length` function.
...
![alt text](<Images/13_TF_vars 2.png>)

![alt text](Images/3a.errors.png)
...
![alt text](Images/18_TF_commands.png)

![alt text](Images/18_TF_commands_2.png)

And after executing `terraform apply` we can see the below screenshot of the created VPC

![alt text](Images/19_new_VPC.png)

And `2 subnets` created shown below:

![alt text](Images/20_new_subnets.png)

**_Experiment:_** We can change the value of `preferred_number_of_public_subnets` variable to `null` and notice how many subnets get created.

- First I will destroy the resources built by executing `terraform destroy`

![alt text](Images/22_null_1.png)

- Change the value of the variable `preferred_number_of_public_subnets` to `null`

![alt text](Images/22_null_2.png)

- Rebuild the infrastructure again by executing `terraform apply`

![alt text](Images/22_null_3.png)
...
![alt text](Images/17_TF_main.png)

- The tags are used to name the resources
...
![alt text](Images/13_TF_vars.png)
...
![alt text](Images/16_TF_tfvar.png)
...
![alt text](Images/21_TF_files.png)
